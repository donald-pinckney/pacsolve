...
extension SolutionGraph {
    private func dotCode() -> String {
        let nodes = self.vertices.enumerated().map { (idx, v) -> String in
            ...
            case let .resolvedPackage(package: p, version: v, data: _):
                attributes = [("label", "\"\(p)\n\(v)\""), ("color", "\"gray88\""), ("fontcolor", "\"gray88\"")]
            }
        ...
        let edges = self.adjacencyLists.flatMap { (fromVert: Int, toVerts: [Int]) in
    ...
    private func drawGraphToFile(_ d: GenDirManager) throws -> URL {
        ...
        let imageFile = try d.newTempFile(ext: "png", contents: nil)
        try shellOut(to: "dot", arguments: ["-Tpng", dotFile.path, "-Gdpi=192", "-o", imageFile.path])
        return imageFile
    }
